...
Camera::Camera(const int width, const int height, const glm::vec3& position) :
	state{ CameraStates::BEHIND_PLANE }
{
...
void Camera::Set(const glm::vec3& position)
{
	this->isPerspective = true;
	this->yaw = YAW;
	this->pitch = PITCH;
...
const glm::mat4 Camera::GetProjectionMatrix() const
{
	...
	if (isPerspective) {
		...
		Proj = glm::perspective(glm::radians(45.0f), (float)width / (float)height, 0.1f, 100000.0f);
	}
	else {
...
{
	if (state == CameraStates::SPECTATOR) {
	...
	else if (state == CameraStates::BEHIND_PLANE) {
		right = glm::normalize(glm::cross(forward, worldUp));  // Normalize the vectors, because their length gets closer to 0 the more you look up or down which results in slower movement.
		up = glm::normalize(glm::cross(right, forward));
	}
}
